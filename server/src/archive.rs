...
impl Archive {
  ...
  async fn retrieve(&self, prefix: &str, file: PathBuf) -> Option<NamedFile> {
    NamedFile::open(Path::new(prefix).join(file)).await.ok()
  }

  pub async fn retrieve_asset(&self, file: PathBuf) -> Option<NamedFile> {
    self.retrieve(&self.asset_dir, file).await
  }

  pub async fn retrieve_note(&self, file: PathBuf) -> Option<NamedFile> {
